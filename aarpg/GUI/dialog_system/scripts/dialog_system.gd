...
@onready var dialog_progress_container: PanelContainer = $DialogUI/DialogProgressContainer
@onready var dialog_progress_container_label: Label = $DialogUI/DialogProgressContainer/Label
...
func show_dialog_button_indicator(_is_visible: bool) -> void:
	dialog_progress_container.visible = _is_visible
	if dialog_item_index + 1 < dialog_items.size():
		dialog_progress_container_label.text = "NEXT"
	else:
		dialog_progress_container_label.text = "END"
